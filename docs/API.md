...
- 健康检查: `GET /health`
- 代理入口: `GET|POST /api/*`
- 鉴权：除 `GET /health` 与 `POST /api/user/login` 外，其他 `/api/*` 路径均要求 `Authorization: Bearer <token>`，由网关校验后转发。

### 路由前缀与后端服务映射
- 用户服务：`/api/user/**` → user-service `/api/**`
- 内容服务：`/api/content/**` → content-service `/api/**`
- 管理服务：`/api/admin/**` → admin-service `/api/**`
- 统计服务：`/api/stat/**` → stat-service `/api/**`

例如：`GET /api/content/article/list` → content-service `GET /api/article/list`

---

## 用户（user）
- 健康检查: `GET /api/user/health`（经网关转发到 user-service `/health`）
...
- 获取文章详情：`GET /api/content/article/:article_id`
  - 响应体示例：
  ```json
...
- 关键词搜索：`GET /api/content/article/search?q=&page=&page_size=`
  - 响应体：同上（返回摘要列表）
- 分类列表（单层，全量）：`GET /api/content/category/list`
  - 响应体：
  ```json
...
- 分页查询：`GET /api/admin/users?page=&page_size=`
  - 响应体：`{ code,message,data:{ list:[], total:0, page:1, page_size:10 } }`
...
- 修改：`POST /api/admin/users/update/:id`
  - 请求体示例：`{"email":"u2@a.com","role":"user"}`
- 删除：`POST /api/admin/users/delete/:id`

### 文章管理（全量列表）
- 列表（全量）：`GET /api/admin/articles`
  - 响应体：`{ code,message,data:{ list:[], total:0, page:1, page_size:<len(list)> } }`
...
- 新增：`POST /api/admin/categories`
  - 请求头：`Content-Type: application/json`
  - 请求体示例：`{"name":"后端","slug":"backend","sort":10}`
- 修改：`POST /api/admin/categories/update/:id`
  - 请求体示例：`{"name":"服务端","sort":20}`
- 删除：`POST /api/admin/categories/delete/:id`
- 分类（扁平树展示）：`GET /api/admin/categories/tree`
  - 响应体：`{ code,message,data:{ list:[{"id":1,"name":"A","slug":"a","sort":10}], total:<n>, page:1, page_size:<len(list)> } }`
...
- PV 时间序列：`GET /api/admin/stat/pv_timeseries?from=2025-08-01T00:00:00Z&to=2025-08-01T12:00:00Z&interval=1h`
  - 响应体：`{ code:0, data: [{"ts":1722470400, "value": 100}] }`
...
- 延迟分位：`GET /api/admin/stat/latency_percentile?from=...&to=...&service=user`
  - 响应体：`{ code:0, data: { "p50":10, "p90":30, "p95":60, "p99":120 } }`
...
- 获取统计：`GET /api/stat/get?type=view&target_id=1&target_type=article[&user_id=2]`
  - 响应体：`{ "code": 0, "message": "success", "data": { "value": 123 } }`
...
4. 文章摘要列表：`GET /api/content/article/list?page=1&page_size=10` → 返回 list,total
5. 关键词搜索：`GET /api/content/article/search?q=go&page=1&page_size=10` → 返回 list,total
6. 分类列表（全量）：`GET /api/content/category/list` → 返回单层分类
...
10. 增加统计值：`POST /api/stat/incr?type=like&target_id=1&target_type=article&user_id=2` → success
